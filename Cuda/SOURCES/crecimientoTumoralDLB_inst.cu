...
__global__ void contarCelulasTumorales(
    const Celula* grid,
    int estado,
    unsigned int* total)
{
    extern __shared__ unsigned int contadorBloque[];
    ...
    contadorBloque[x] = grid[indice].cancer ? 1U : 0U;
    __syncthreads();

    // Reducción dentro de cada bloque.
    for (unsigned int salto = blockDim.x / 2;
         salto > 0;
         salto >>= 1)
    {
        if (x < salto) {
            contadorBloque[x] += contadorBloque[x + salto];
        }
        __syncthreads();
    }

    // Solo una operación atómica por bloque.
    if (x == 0) {
        atomicAdd(total, contadorBloque[0]);
    }
}
...
unsigned int* d_num_celulas = nullptr;
cudaMalloc(&d_num_celulas, sizeof(unsigned int));

unsigned int h_num_celulas = 0;
...
ofstream evolution_file("tumor_evolution.csv");

if (!evolution_file.is_open()) {
    cerr << "No se puede abrir tumor_evolution.csv" << endl;
    cudaFree(d_num_celulas);
    cudaFree(d_grid);
    return -1;
}

evolution_file << "iteration,day,step,tumor_cells\n";	
...
const int estado_siguiente = (indice_rejilla + 1) % 2;

cudaMemset(d_num_celulas, 0, sizeof(unsigned int));

contarCelulasTumorales<<<LONG, LONG,
